...
var _timestampConversion = require("../utils/timestampConversion");
...
function convertTimestampInCredentials(rawCredentials) {
  let expiresAt = (0, _timestampConversion.convertExpiresInToExpiresAt)(rawCredentials.expiresIn);
  if (!expiresAt) {
...
class Auth {
  ...
  authorizeUrl(parameters) {
    const query = (0, _whitelist.apply)({
      parameters: {
        redirectUri: {
          required: true,
          toName: 'redirect_uri'
   ...
   */
  exchange(parameters) {
    const payload = (0, _whitelist.apply)({
      parameters: {
        code: {
          required: true
        },
    ...
    }).then(response => convertTimestampInCredentials(responseHandler(response)));
  }
  ...
  exchangeNativeSocial(parameters) {
    ...
    return this.client.post('/oauth/token', {
      ...
      return convertTimestampInCredentials(responseHandler(response));
    });
  }
  ...
  userInfo(parameters) {
    ...
    const client = new _networking.default({
      baseUrl,
      telemetry,
